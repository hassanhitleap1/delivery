                        ...
                        <div class="card-body table-responsive p-2">
                            <table class="table table-hover">
                                ...
                                <tr v-for="(wallet,index) in wallets.data" :key="index">
                                    <td>{{wallet.id}}</td>
                                    <td>{{wallet.driver.name}}</td>
                                    <td>{{wallet.wallet}}</td>
                                    <td class="action">
                                        <router-link class="tag tag-success fas fa-edit"  :to="{'name':'driverwallet.edit',params:{'id':wallet.id}}" >
                                        </router-link>
                                    </td>
                                </tr>
                                </tbody>
                            ...
                            <pagination align="center" :data="wallet" @pagination-change-page="get_wallets"></pagination>
                        </div>
                        <!-- /.card-body -->
                    </div>
                    <!-- /.card -->
