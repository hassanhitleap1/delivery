<template>
    <layout name="LayoutDefault">
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <!-- left column -->
                <div v-if="errors">
                    <div v-for="(v, k) in errors" :key="k">
                        <p v-for="error in v" :key="error" class="alert alert-danger" role="alert">
                            {{ error }}
                        </p>
                    </div>
                </div>
                <div class="col-md-12">
                    <!-- general form elements -->
                    <div class="card card-primary">
                        <div class="card-header">
                            <h3 class="card-title float-left">update {{statu.name}}</h3>
                        </div>
                        <!-- /.card-header -->



                        <form role="form" @submit.prevent="update(statu,id)" >
                            <div class="card-body">
                                <div class="form-group">
                                    <label for="name">name</label>
                                    <input type="text" class="form-control" id="name" placeholder="Enter name" v-model="statu.name">
                                </div>
                            </div>
                            <!-- /.card-body -->

                            <div class="card-footer">
                                <button type="submit" class="btn btn-primary">Submit</button>
                            </div>
                        </form>
                    </div>
                    <!-- /.card -->

                </div>
            </div>
        </div>
    </section>
    </layout>
</template>

<script>

import  * as services from '../../services/status';
import Layout from "../layouts/Layout";
export default {
    name: "Edit",
    components:{
        Layout,
    },
    data(){
        return {
            errors: [],
            success : false,
            id:null,
            statu:{},
        }
    },
    mounted() {
        this.id=this.$route.params.id;
        services.get_statu(this.id).then( response => {
            this.statu=response.data.data;
        }).catch((error) => {
            console.log("error",error)
        });

    },methods:{
        update(statu,id) {
            services.update(statu).then( response => {
                this.$store.dispatch('StatusModule/updateStatuStatu',statu);
                this.errors = [];
                this.success = true;
                this.$router.push({ 'name': 'status.index' });
            }).catch((error) => {
                this.errors = error.response.data.errors;
                this.success = false;
            });

        }
    },


}
</script>


